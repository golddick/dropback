...
export default async function ProjectPage({ params }: { params: { projectId: string } }) {
  ...
  // Check membership but don't block viewing - just track for UI purposes
  let isMember = false;
  if (isAuthenticated) {
    try {
      await requireMembership(projectId, userId);
      ...
      // User is authenticated but not a member - they can still view
      isMember = false;
    }
  }
        ...
        <div>
          ...
          <div className="mt-2 gap-2 space-y-3">
            {!isAuthenticated ? (
              ...
              <div className="rounded-lg border border-hairline bg-surface px-4 py-3 text-sm text-muted">
                View-only mode. You are not a member of this project.
              </div>
            ) : (
              <>
                <InviteMemberForm projectId={projectId} />
